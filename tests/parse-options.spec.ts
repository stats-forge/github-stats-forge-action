...
describe(parseOptions, () => {
  it('reads a query string, which is what a README URL already contains', () => {
    expect(parseOptions('username=octocat&theme=dark')).toStrictEqual({
      username: 'octocat',
      theme: 'dark',
    });
  });
  ...
  it('accepts the leading ? of a pasted URL query', () => {
    expect(parseOptions('?username=octocat')).toStrictEqual({ username: 'octocat' });
  });

  it('joins a repeated key the way the renderer expects a list', () => {
    expect(parseOptions('hide=stars&hide=issues')).toStrictEqual({
      hide: 'stars,issues',
    });
  });

  it('decodes percent-encoded values', () => {
    expect(parseOptions('custom_title=My%20Stats')).toStrictEqual({
      custom_title: 'My Stats',
    });
  });

  it('keeps a key that was given no value', () => {
    expect(parseOptions('username=octocat&show_icons')).toStrictEqual({
      username: 'octocat',
      show_icons: '',
    });
  });
  ...
  it('reads a JSON object, for options that are awkward in a query string', () => {
    expect(parseOptions('{"username":"octocat","show_icons":true}')).toStrictEqual({
      username: 'octocat',
      show_icons: 'true',
    });
  });
  ...
  it('joins a JSON array into the comma list the renderer parses', () => {
    expect(parseOptions('{"hide":["stars","issues"]}')).toStrictEqual({
      hide: 'stars,issues',
    });
  });

  it('drops null and undefined, so an unset YAML value is not sent as text', () => {
    expect(parseOptions('{"username":"octocat","theme":null}')).toStrictEqual({
      username: 'octocat',
    });
  });

  it('keeps a JSON false, which is not the same as unset', () => {
    expect(parseOptions('{"show_icons":false}')).toStrictEqual({
      show_icons: 'false',
    });
  });
